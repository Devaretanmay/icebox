"""Python bindings for the ICEBOX Governance SDK (C ABI / ctypes).

Loads ``libicebox_capi`` and wraps the ``extern "C"`` functions so any Python
agent can be governed by the same charter / scope / risk / approval gates that
guard native ICEBOX modules.
"""
...
_LIB_NAME = "icebox_capi"
_CANDIDATES = [
    "target/debug/libicebox_capi.dylib",
    "target/debug/libicebox_capi.so",
    "target/release/libicebox_capi.dylib",
    "target/release/libicebox_capi.so",
    "libicebox_capi.dylib",
    "libicebox_capi.so",
    "icebox_capi.dll",
...
def _load_lib():
    for path in _CANDIDATES:
        ...
        try:
            ...
            continue
    raise RuntimeError(
        "libicebox_capi not found. Build it with `cargo build -p icebox-capi` "
