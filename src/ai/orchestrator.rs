...
mod tests {
    ...
    async fn direct_seam_records_audit_and_allows_low_risk() {
        ...
        assert!(
            res.is_ok(),
            "in-scope, approved, low-risk run must pass the seam"
        );
    ...
    async fn destructive_requires_approval_and_is_recorded() {
        ...
        assert!(
            res.is_err(),
            "destructive module without approval must be denied"
        );
        let decs = fw.lock().await.executor.recent_decisions(10);
        assert!(
            decs.iter()
                .any(|d| matches!(d.decision, PolicyDecision::RequireApproval(_))),
