...
//! Run with `cargo test --test evidence`.

use icebox::core::safety::{normalize_evidence, Evidence, EvidenceProvenance};

#[test]
fn structured_json_is_high_confidence_and_normalized() {
    let (kind, conf, norm) =
        normalize_evidence(r#"{"port":22,"service":"ssh"}"#, "tcp_port_scanner");
...
fn keyword_hints_set_kind_and_confidence() {
    assert_eq!(
        normalize_evidence("found open port 443", "x").0.as_deref(),
        Some("port")
    );
    assert_eq!(normalize_evidence("found open port 443", "x").1, 0.65);

    assert_eq!(
        normalize_evidence("credential admin:admin leaked", "x")
            .0
            .as_deref(),
        Some("credential")
    );
    assert_eq!(
        normalize_evidence("credential admin:admin leaked", "x").1,
        0.7
    );

    assert_eq!(
        normalize_evidence("cve-2024-1234 present", "x")
            .0
            .as_deref(),
        Some("vulnerability")
    );
