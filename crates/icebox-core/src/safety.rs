...
use crate::module::{Capability, Intent, ModuleKind};
...
impl PolicyEngine for DefaultPolicy {
    fn evaluate(&self, req: &PolicyRequest) -> PolicyDecision {
        if !req.charter_accepted {
            return PolicyDecision::Deny("charter not accepted - run `charter accept` first".into());
        }
        if !req.in_scope {
...
impl PolicySet {
    ...
    pub fn has_cvss_rules(&self) -> bool {
        self.rules.iter().any(|r| matches!(r, PolicyRule::DenyIfCvssAbove(_) | PolicyRule::RequireApprovalIf { .. }))
    }
}
...
impl RiskEvaluator for DefaultRiskEvaluator {
    ...
    fn evaluate_raw(&self, contents: &[String], _kind: &Option<String>) -> Option<CvssScore> {
        contents
            ...
            .max_by(|a, b| a.weighted_risk().partial_cmp(&b.weighted_risk()).unwrap_or(std::cmp::Ordering::Equal))    }
}
...
pub fn normalize_evidence(content: &str, _module: &str) -> (Option<String>, f64, Option<Value>) {
    if let Ok(Value::Object(map)) = serde_json::from_str::<Value>(content) {
        return (infer_kind(map.keys().map(|k| k.as_str())), 0.9, Some(Value::Object(map)));
    }
...
fn infer_kind<'a>(keys: impl Iterator<Item = &'a str>) -> Option<String> {
    ...
    if lower
        .iter()
        .any(|k| k.contains("credential") || k.contains("password") || k.contains("user") || k.contains("hash"))
    {
        Some("credential".into())
    } else if lower.iter().any(|k| k.contains("cve") || k.contains("vuln")) {
        Some("vulnerability".into())
    } else if lower.iter().any(|k| k.contains("port") || k.contains("service")) {
        Some("service".into())
    } else {
        Some("finding".into())
    }
