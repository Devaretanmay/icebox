...
use crate::job::{Job, JobId, JobManager, JobStatus};
use crate::module::ModuleResult;
use crate::governance::PolicyPackStore;
use crate::safety::{Charter, MemoryEntry, PolicySet, RiskLevel, ScopeManager};
use crate::session::{Session, SessionId, SessionKind, SessionManager};
...
impl WorkspaceSnapshot {
    ...
    pub fn save_to_file(&self, path: &str) -> Result<(), crate::WorkspaceError> {
        let json = serde_json::to_string_pretty(self).map_err(crate::WorkspaceError::Json)?;
        std::fs::write(path, &json).map_err(crate::WorkspaceError::Io)?;
        Ok(())
    }

    pub fn load_from_file(path: &str) -> Result<Self, crate::WorkspaceError> {
        let json = std::fs::read_to_string(path).map_err(crate::WorkspaceError::Io)?;
        let snap: WorkspaceSnapshot =
            serde_json::from_str(&json).map_err(crate::WorkspaceError::Json)?;
        Ok(snap)
    }
