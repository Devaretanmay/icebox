...
use crate::module::{LoadedModule, ModuleError, ModuleResult};
use crate::safety::{
    is_destructive, make_config_policy, now_secs, Charter, ConfigPolicy, DecisionRecord, Evidence,
    MemoryEntry, MemoryKind, PolicySet, Preflight, PreflightError, PolicyContext, PolicyEngine,
...
impl ModuleExecutor {
    ...
    fn record_decision(&mut self, module: &str, pf: &Preflight, decision: &crate::safety::PolicyDecision) {
    ...
    fn record_evidence(&mut self, module: &str, target: &str, result: &ModuleResult, job_id: Option<u64>) {
        let seq_start = self.evidence.len();
        for (i, content) in result.evidence.iter().enumerate() {
            self.evidence.push(Evidence::new(module, target, content, job_id, seq_start + i));
        }
