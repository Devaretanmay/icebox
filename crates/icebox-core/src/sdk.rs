...
use crate::governance::{ApprovalRequest, ApprovalQueue, ApprovalStatus, Role};
use crate::module::{Capability, Intent};
use crate::safety::{
    Charter, ConfigPolicy, CvssScore, DecisionRecord, PolicyContext, PolicyDecision,
    PolicyEngine, PolicyRequest, PolicyRule, PolicySet, RiskLevel, ScopeManager, now_secs,
};
...
pub enum GovernedOutcome {
    Allowed { result: Value, decision_id: u64 },
    Blocked { reason: String, decision_id: u64 },
    NeedsApproval {
        reason: String,
        decision_id: u64,
        approval_id: u64,
...
impl GovernanceRuntime {
    ...
    async fn enforce<F, Fut>(&self, task: TaskSpec, action: F, auto_approve: bool) -> GovernedOutcome
    where
        F: FnOnce() -> Fut,
        Fut: std::future::Future<Output = Result<Value, String>> + Send + 'static,
    {
        ...
        match decision {
            PolicyDecision::Deny(reason) => GovernedOutcome::Blocked { reason, decision_id },
            ...
            PolicyDecision::RequireApproval(_) | PolicyDecision::Allow => {
                drop(st);
                match action().await {
                    Ok(result) => GovernedOutcome::Allowed { result, decision_id },
    ...
    pub async fn export_audit_csv(&self) -> String {
        let d = self.state.lock().await.decisions.clone();
        crate::governance::audit_to_csv(&d)
    }
...
impl RuntimeState {
    fn record_and_block(&mut self, task: TaskSpec, reason: String) -> GovernedOutcome {
        ...
        GovernedOutcome::Blocked { reason, decision_id }
    }
}
...
impl GovernanceBuilder {
    ...
    pub fn deny_capability(mut self, cap: Capability) -> Self {
        self.config.policy_set.add_rule(PolicyRule::DenyCapability(cap));
        self
    }

    pub fn allow_capability(mut self, cap: Capability) -> Self {
        self.config.policy_set.add_rule(PolicyRule::AllowCapability(cap));
        self
    }

    pub fn require_approval(mut self, cap: Capability, target_pattern: &str) -> Self {
        self.config.policy_set.add_rule(PolicyRule::RequireApproval {
            capability: cap,
    ...
    pub fn max_risk_ceiling(mut self, max_risk: RiskLevel) -> Self {
        self.config.policy_set.add_rule(PolicyRule::MaxRisk(max_risk));
        self
    }
    ...
    pub fn require_approval_if(
        ...
        self.config.policy_set.add_rule(PolicyRule::RequireApprovalIf {
            cvss_above,
            epss_above,
            kev,
        });
    ...
    pub fn rate_limit(mut self, name: &str, per_minute: u64) -> Self {
        self.config
            .rate_limits
            .insert(name.to_string(), per_minute);
        self
    }
...
mod tests {
    ...
    async fn supervised_low_risk_runs_and_audits() {
        let rt = base_runtime();
        let out = rt
            .execute(low_risk_task("10.0.0.5"), || async { Ok(json!({"open_ports": [22, 80]})) })
            .await;
        match out {
    ...
    async fn destructive_requires_approval_then_runs_after_approve() {
        ...
        let out = rt.execute(task.clone(), || async { Ok(json!({"wiped": true})) }).await;
        let id = match out {
