...
use icebox_core::framework::{new_shared_framework, SharedFramework};
use icebox_core::governance::{ApprovalQueue, PolicyPack, Role};
use icebox_core::job::Job;
use icebox_core::module::{Capability, LoadedModule, ModuleResult};
use icebox_core::safety::{
    Charter, CvssScore, DecisionRecord, DefaultRiskEvaluator, MemoryKind, PolicyContext,
    PolicyDecision, PolicyEngine, PolicyRule, ReasoningTrace, RiskEvaluator,
    RiskLevel, ScopeManager, now_secs,
};

use icebox_core::workspace::WorkspaceSnapshot;
...
async fn setup_module(fw: &SharedFramework) -> LoadedModule {
    ...
    let mut loaded = icebox_modules::load("tcp_port_scanner").expect("module must be available");
...
async fn run_through_seam(
    ...
    target: &str,
    approved: bool,
) -> (Result<ModuleResult, icebox_core::executor::ExecutorError>, usize) {
    ...
    let result = g
        .executor
        .execute(loaded, target, None, approved, PolicyContext::Autonomous, Some(jid.as_u64()))
        .await;
    let decisions_after = g.executor.decisions.len();
    match &result {
        Ok(r) => { g.jobs.complete(jid, r.clone()); }
        Err(_) => { g.jobs.cancel(jid); }
    }
...
async fn seam_bypass_direct_module_run_skips_policy() {
    ...
    let g = fw.lock().await;
    assert_eq!(g.executor.decisions.len(), 0, "bypass leaves no audit trail");
    assert_eq!(g.executor.evidence.len(), 0, "bypass leaves no evidence trail");
}

#[tokio::test]
async fn leaked_loaded_module_bypasses_all_gates() {
    let mut loaded = icebox_modules::load("reverse_shell_payload").expect("module available");
    ...
    let fw = base_framework();
    {
        ...
        let pf = g.executor.preflight(&loaded, "127.0.0.5", None, false, PolicyContext::Autonomous);
...
async fn policy_deny_always_wins_over_approval() {
    ...
    let loaded = icebox_modules::load("ssh_bruteforce").expect("module available");
    let pf = {
        let g = fw.lock().await;
        g.executor.preflight(&loaded, "10.0.0.5", None, true, PolicyContext::Autonomous)
    };
    let policy = {
        let g = fw.lock().await;
...
async fn viewer_role_cannot_execute_via_rbac() {
    ...
    assert!(
        !icebox_core::governance::role_allows(role, Role::Operator),
...
fn module_capability_declarations_are_honest() {
    let loaded = icebox_modules::load("ssh_bruteforce").expect("module available");
    assert!(
        loaded.info.capabilities.contains(&Capability::CredentialAccess),
    ...
    let scan = icebox_modules::load("tcp_port_scanner").expect("module available");
    assert!(
        !scan.info.capabilities.contains(&Capability::CredentialAccess),
...
async fn options_cannot_override_policy() {
    ...
    let pf = {
        let g = fw.lock().await;
        g.executor.preflight(&loaded, "10.0.0.99", None, false, PolicyContext::Autonomous)
    };

    assert!(pf.risk >= RiskLevel::High, "payload impact must remain High regardless of options");
...
async fn every_execution_is_audited() {
    ...
    assert_eq!(added, 1, "each execute must add exactly one decision record");
    ...
    assert!(matches!(last.decision, PolicyDecision::Allow), "in-scope approved should be allowed");
}
...
async fn audit_trail_links_decisions_to_evidence_to_preflight() {
    ...
    let preflight = {
        let g = fw.lock().await;
        g.executor.preflight(&loaded, target, None, false, PolicyContext::Autonomous)
    };
...
async fn denied_decisions_include_reason() {
    ...
    let pf = {
        let g = fw.lock().await;
        g.executor.preflight(&loaded, "10.0.0.5", None, false, PolicyContext::Autonomous)
    };
    let policy = {
        let g = fw.lock().await;
        g.executor.policy(PolicyContext::Autonomous)
...

    {
        ...
        match &recorded.decision {
            PolicyDecision::Deny(reason) => {
                ...
                assert!(reason.contains("charter"), "denied decision reason must explain why");
            }
...
fn approval_queue_handles_bulk() {
    ...
    let all = q.list();
    assert_eq!(all.len(), count, "all {count} approval requests must be stored");
    assert_eq!(
        all.iter().filter(|r| r.status == icebox_core::governance::ApprovalStatus::Pending).count(),
        count,
    ...
    let approved_count = final_list.iter()
        .filter(|r| r.status == icebox_core::governance::ApprovalStatus::Approved)
        .count();
    let denied_count = final_list.iter()
        .filter(|r| r.status == icebox_core::governance::ApprovalStatus::Denied)
        .count();
    assert_eq!(approved_count, count / 2 + count % 2, "expected ~half approved");
    assert_eq!(denied_count, count / 2, "expected ~half denied");

    assert!(!q.approve(0), "already approved request cannot be re-approved");
...
async fn full_traceability_chain() {
    ...
    if !g.executor.evidence.is_empty() {
        ...
        assert!(ev.provenance.job_id.is_some(), "evidence must link to a job");
        assert!(ev.confidence > 0.0, "evidence must have confidence score");
        assert_eq!(ev.module, "tcp_port_scanner");
    }

    assert!(!g.executor.decisions.is_empty(), "at least one decision must be recorded");
...
async fn operator_can_inspect_state() {
    ...
    assert!(
        matches!(recent_jobs[0].status, icebox_core::job::JobStatus::Completed | icebox_core::job::JobStatus::Failed),
        "job must have completed or failed"
    );

    assert!(g.executor.policy_set.version >= 1, "policy version must be >= 1");
}
...
fn continuous_validation_diff_detects_policy_drift() {
    use icebox_ai::diff;
    use icebox_ai::CampaignReport;
    use icebox_ai::ValidationReport;
    ...
    assert_eq!(d.policy_version_b, 3, "diff must detect policy version change");
...
fn validation_report_is_serializable() {
    ...
    assert!(json.contains("\"policy_version\": 2"), "policy version must be in serialized form");
    assert!(json.contains("\"ran_at\": 12345"), "timestamp must be in serialized form");
...
async fn multi_agent_policy_is_coherent() {
    ...
    for i in 0..3 {
        let pf = {
            let g = fw.lock().await;
            g.executor.preflight(&loaded, "127.0.0.1", None, true, PolicyContext::Autonomous)
        };
        let policy = {
            let g = fw.lock().await;
            g.executor.policy(PolicyContext::Autonomous)
...
async fn orchestrated_agents_share_audit_trail() {
    use icebox_ai::Orchestrator;
    use icebox_ai::StaticPlanner;
    ...
    let report = orch
        .run(&["127.0.0.1".to_string(), "127.0.0.1".to_string()], || Box::new(StaticPlanner::new()))
        .await;

    let g = fw.lock().await;
    assert!(report.total_jobs > 0, "orchestrated agents must produce jobs");
    assert!(
        g.executor.recent_decisions(50).len() >= 2,
        "both agents must produce decisions in the shared audit trail"
    );
    assert!(report.total_traces > 0, "orchestrated agents must produce traces");
}
...
async fn workspace_preserves_all_governance_state() {
...

    {
        ...
        g.executor
            .remember(MemoryKind::Fact, "test memory");
    }
    ...
    assert_eq!(snap.policy_rules.rules.len(), 1, "policy rule must be preserved");
    assert!(snap.policy_packs.contains_key("test-pack"), "policy packs must be preserved");
...
fn empty_scope_blocks_everything() {
    ...
    assert!(!scope.is_in_scope("10.0.0.1"), "empty scope must block everything");
    assert!(!scope.is_in_scope("anything"), "empty scope must block everything");
}
...
fn scope_wildcards_work() {
    ...
    assert!(scope.is_in_scope("10.0.0.1"), "wildcard should match subnet");
    assert!(scope.is_in_scope("192.168.1.100"), "exact match should work");
    assert!(!scope.is_in_scope("10.0.1.1"), "different subnet should not match");
    assert!(!scope.is_in_scope("192.168.1.101"), "non-wildcard exact should not match");
}
...
fn scope_cidr_works() {
    ...
    assert!(scope.is_in_scope("10.0.0.255"), "CIDR /24 should match .255");
    assert!(!scope.is_in_scope("10.0.1.1"), "CIDR /24 should not match different subnet");
}

#[test]
fn destructive_keywords_are_detected() {
    use icebox_core::safety::is_destructive;

    assert!(is_destructive("wipe_disk"), "wipe must be destructive");
    assert!(is_destructive("format_volume"), "format must be destructive");
    assert!(is_destructive("destroy_everything"), "destroy must be destructive");
    assert!(is_destructive("delete_all"), "delete must be destructive");
    assert!(is_destructive("shutdown_system"), "shutdown must be destructive");
    assert!(is_destructive("reboot_server"), "reboot must be destructive");
    assert!(is_destructive("bruteforce_login"), "bruteforce must be destructive");
    assert!(is_destructive("ransom_note"), "ransom must be destructive");

    assert!(!is_destructive("scan_ports"), "scan must not be destructive");
    assert!(!is_destructive("resolve_dns"), "dns must not be destructive");
    assert!(!is_destructive("probe_http"), "probe must not be destructive");
}
...
fn default_risk_evaluator_parses_evidence() {
    ...
    assert!((score.effective_score() - 9.8).abs() < 0.01, "expected CVSS 9.8, got {}", score.effective_score());
    assert!(score.kev, "CVE-2024-1234 must be KEV");
    assert!((score.epss.unwrap_or(0.0) - 0.95).abs() < 0.01, "expected EPSS 0.95");

    assert!((score.weighted_risk() - 10.0).abs() < 0.01, "weighted risk should be clamped to 10.0");
    assert_eq!(score.severity(), RiskLevel::Critical, "CVSS 9.8 must be Critical");

    let no_vuln = evaluator.evaluate(&evidence[2..]);
    assert!(no_vuln.is_none(), "non-CVSS evidence should not produce a score");
}
...
fn cvss_weighted_risk_blends_all_factors() {
    ...
    assert!((s3.weighted_risk() - 10.0).abs() < 0.01, "KEV should push to 10.0");
}
...
async fn cvss_multi_condition_require_approval_if() {
    ...
    ));
    {
        let mut g = fw.lock().await;
        g.operator_role = Role::Operator;
        g.executor.policy_set.add_rule(PolicyRule::RequireApprovalIf {
            cvss_above: Some(8.0),
            epss_above: Some(0.3),
            kev: true,
        });
    ...
    let loaded = icebox_modules::load("tcp_port_scanner").expect("module available");

    let decision = {
        let g = fw.lock().await;
        let mut req = g.executor.preflight(&loaded, "10.0.0.5", None, false, PolicyContext::Autonomous).to_request();
    ...
    assert!(matches!(decision, PolicyDecision::RequireApproval(_)), "CVSS alone must trigger: got {decision:?}");
    ...
    assert!(matches!(decision, PolicyDecision::RequireApproval(_)), "EPSS alone must trigger: got {decision:?}");
    ...
    assert!(matches!(decision, PolicyDecision::RequireApproval(_)), "KEV alone must trigger: got {decision:?}");

    let decision = {
        let g = fw.lock().await;
        let mut req = g.executor.preflight(&loaded, "10.0.0.5", None, true, PolicyContext::Autonomous).to_request();
        req.cvss = Some(CvssScore {
            cvss_v31: Some(5.0),
            cvss_v40: None,
            epss: Some(0.1),
    ...
    assert!(matches!(decision, PolicyDecision::Allow), "no trigger must allow: got {decision:?}");
}

#[tokio::test]
async fn governance_runtime_enforces_cvss_rules() {
    use icebox_core::{GovernanceRuntime, TaskSpec};
    use serde_json::json;
    ...
    let outcome = rt.execute(task, || async { Ok(json!({"exploited": true})) }).await;
    assert!(
        matches!(outcome, icebox_core::GovernedOutcome::Blocked { .. }),
    ...
    let outcome = rt.execute(task, || async { Ok(json!({"exploited": true})) }).await;
    assert!(
        matches!(outcome, icebox_core::GovernedOutcome::NeedsApproval { .. }),
    ...
    assert!(
        matches!(outcome, icebox_core::GovernedOutcome::Allowed { .. }),
...
async fn cvss_appears_in_decision_context() {
    ...
    let _ = rt.execute(task, || async { Ok(json!({"result": true})) }).await;

    let audit = rt.audit().await;
    let blocked = audit.iter().find(|d| matches!(d.decision, icebox_core::PolicyDecision::Deny(_)));
    assert!(blocked.is_some(), "must have a blocked decision record");
    if let Some(rec) = blocked {
        if let icebox_core::PolicyDecision::Deny(reason) = &rec.decision {
            assert!(reason.contains("CVSS"), "denied decision reason must mention CVSS: {reason}");
        }
    }
}
...
async fn governed_vuln_scan_blocks_high_cvss_exploit() {
    use icebox_core::{GovernanceRuntime, GovernedOutcome, TaskSpec};
    use serde_json::json;
    ...
    let mut loaded = icebox_modules::load("vuln_scanner")
        .expect("vuln_scanner module must be available");
    loaded
        .module
    ...
    let blocked = audit
        .iter()
        .find(|d| matches!(d.decision, icebox_core::PolicyDecision::Deny(_)));
    assert!(
        blocked.is_some(),
