...
//! approval queue, audit export). Run with `cargo test -p icebox-core --test governance`.

use icebox_core::Capability;
use icebox_core::governance::{ApprovalQueue, PolicyPack, Role, audit_to_csv, role_allows};
use icebox_core::safety::{
    CvssScore, DecisionRecord, PolicyContext, PolicyDecision, PolicyRequest, PolicyRule,
    RiskLevel, target_matches,
};
...
fn approval_queue_lifecycle() {
    ...
    let a = q.request("exploit".into(), "10.0.0.1".into(), "authorized test".into(), Default::default());
    let b = q.request("scanner".into(), "10.0.0.2".into(), "routine".into(), Default::default());
    assert_eq!(a, 0);
    assert_eq!(b, 1);
    ...
    assert!(q.approve(a));
    assert!(q.deny(b));
    assert_eq!(q.get(a).unwrap().status, icebox_core::governance::ApprovalStatus::Approved);
    assert_eq!(q.get(b).unwrap().status, icebox_core::governance::ApprovalStatus::Denied);
...
fn cvss_deny_above_threshold() {
    use icebox_core::safety::PolicyEngine;
    let cvss_high = CvssScore { cvss_v31: Some(9.1), cvss_v40: None, epss: None, kev: false };
    let cvss_low = CvssScore { cvss_v31: Some(4.5), cvss_v40: None, epss: None, kev: false };

    let policy = icebox_core::safety::ConfigPolicy {
        max_risk: RiskLevel::Critical,
        context: PolicyContext::Autonomous,
        rules: icebox_core::safety::PolicySet { rules: vec![PolicyRule::DenyIfCvssAbove(7.0)], version: 1 },
    };
    ...
    let req_low = PolicyRequest { cvss: Some(cvss_low), ..req };
...
fn cvss_require_approval_above_threshold() {
    use icebox_core::safety::PolicyEngine;
    let cvss_high = CvssScore { cvss_v31: Some(8.5), cvss_v40: None, epss: None, kev: false };
    let cvss_low = CvssScore { cvss_v31: Some(3.0), cvss_v40: None, epss: None, kev: false };

    let policy = icebox_core::safety::ConfigPolicy {
        max_risk: RiskLevel::Critical,
        context: PolicyContext::Autonomous,
        rules: icebox_core::safety::PolicySet {
            rules: vec![PolicyRule::RequireApprovalIf { cvss_above: Some(7.0), epss_above: None, kev: false }],
            version: 1,
        },
    };
    let req = PolicyRequest {
    ...
    assert!(matches!(policy.evaluate(&req), PolicyDecision::RequireApproval(_)));
...
fn cvss_epss_triggers_approval() {
    use icebox_core::safety::PolicyEngine;
    let cvss = CvssScore { cvss_v31: Some(4.0), cvss_v40: None, epss: Some(0.8), kev: false };

    let policy = icebox_core::safety::ConfigPolicy {
        max_risk: RiskLevel::Critical,
        context: PolicyContext::Autonomous,
        rules: icebox_core::safety::PolicySet {
            rules: vec![PolicyRule::RequireApprovalIf { cvss_above: None, epss_above: Some(0.5), kev: false }],
            version: 1,
        },
    };
    let req = PolicyRequest {
...
fn cvss_kev_triggers_approval() {
    ...
    let policy = icebox_core::safety::ConfigPolicy {
        ...
        context: PolicyContext::Autonomous,
        rules: icebox_core::safety::PolicySet {
            rules: vec![PolicyRule::RequireApprovalIf { cvss_above: None, epss_above: None, kev: true }],
            version: 1,
        },
    };
    let req = PolicyRequest {
...
fn audit_csv_has_header_and_escapes_commas() {
    ...
    assert_eq!(lines[0], "at,target,module,capabilities,intents,impact,context,decision,reason");
