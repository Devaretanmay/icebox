...
use icebox::core::safety::{Charter, ScopeManager, RiskLevel, PolicyContext};
...
impl NativeIcebox {
    ...
    fn run_module(
        ...
        let result_json = py.allow_threads(move || {
            ...
            if let Some(evidence) = result_out.get_mut("evidence").and_then(|e| e.as_array_mut()) {
            ...
            Ok::<String, PyErr>(serde_json::to_string(&result_out).map_err(|e| PyRuntimeError::new_err(e.to_string()))?)
        })?;
