...
const KEV_CVES: &[&str] = &[
    ...
    "CVE-2022-0847",  "CVE-2022-30190",
    "CVE-2023-34362", "CVE-2023-2868", "CVE-2023-35078",
    "CVE-2023-3519",  "CVE-2023-4966", "CVE-2023-46604",
    "CVE-2023-6345",  "CVE-2023-7024",
    "CVE-2024-3094",  "CVE-2024-1709", "CVE-2024-23897",
];

fn is_kev(cve_id: &str) -> bool {
    let upper = cve_id.to_uppercase();
    KEV_CVES.iter().any(|k| upper == *k || upper.contains(k.trim_start_matches("CVE-")))
}
...
pub struct VulnScanner {
    #[option(required = true, help = "Path to project directory containing Cargo.toml")]
    pub project_dir: String,
    #[option(help = "Timeout per API call in milliseconds (default 15000)")]
    pub timeout_ms: u64,
    #[option(help = "Enable watch mode: re-check dependencies periodically (true/false, default: false)")]
    pub watch: bool,
    #[option(help = "Interval between watch-mode scans in seconds (default: 3600)")]
    pub watch_interval_secs: u64,
    #[option(help = "Number of watch-mode scans to run (default: 3, 0 = infinite, use with caution)")]
...
impl Module for VulnScanner {
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        let timeout = Duration::from_millis(if self.timeout_ms > 0 { self.timeout_ms } else { 15000 });
        let project_dir = self.project_dir.trim().to_string();
        let watch = self.watch;
        let interval = Duration::from_secs(if self.watch_interval_secs > 0 { self.watch_interval_secs } else { 3600 });
        let max_scans = if watch && self.watch_scans == 0 { usize::MAX } else if watch { self.watch_scans.max(1) } else { 1 };
        ...
        let critical_count = all_findings_deduped.iter().filter(|f| f.severity == "critical").count();
        let high_count = all_findings_deduped.iter().filter(|f| f.severity == "high").count();
...
async fn fetch_cargo_metadata(project_dir: &str) -> Result<Vec<CargoPackage>, ModuleError> {
    ...
    if !output.status.success() {
        ...
        return Err(ModuleError::Other(format!("cargo metadata error: {stderr}")));
    }
...
async fn query_osv(
    ...
    if !resp.status().is_success() {
        return Err(format!("OSV query for {name}@{version}: HTTP {}", resp.status()));
    }
...
mod tests {
    ...
    fn test_format_new_evidence() {
        ...
        assert!(ev.contains("[NEW]"), "new evidence should have [NEW] tag: {ev}");
    ...
    fn test_delta_detection_basic() {
        ...
        let c0_findings = vec![
            VulnerabilityFinding {
                cve: "CVE-2024-001".into(), package: "crate_a".into(),
                installed_version: "1.0".into(), fixed_version: None,
                summary: "".into(), cvss_v31: Some(5.0), epss: None, kev: false,
                new_finding: false, severity: "medium".into(),
            },
            VulnerabilityFinding {
                cve: "CVE-2024-002".into(), package: "crate_b".into(),
                installed_version: "1.0".into(), fixed_version: None,
                summary: "".into(), cvss_v31: Some(3.0), epss: None, kev: false,
                new_finding: false, severity: "low".into(),
            },
        ];
        ...
        let c1_findings = vec![
            ...
            VulnerabilityFinding {
                cve: "CVE-2024-003".into(), package: "crate_c".into(),
                installed_version: "1.0".into(), fixed_version: None,
                summary: "".into(), cvss_v31: Some(9.0), epss: None, kev: false,
                new_finding: true, severity: "critical".into(),
            },
        ];
        ...
        // Cycle 2: no new CVEs
        let c2_findings: Vec<VulnerabilityFinding> = vec![
            VulnerabilityFinding {
                cve: "CVE-2024-003".into(), package: "crate_c".into(),
                ..Default::default()
            },
    ...
    async fn test_osv_query_no_vulns() {
        ...
        let findings = query_osv(&client, "zzzz_invalid_crate_000", "0.1.0", Duration::from_secs(10))
            .await
            .expect("OSV query must succeed");

        assert!(findings.is_empty(), "non-existent crate should have no CVEs");
    }
