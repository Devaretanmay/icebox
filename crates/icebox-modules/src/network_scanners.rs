...
pub struct ArpScanner {
    #[option(required = true, help = "Target range: CIDR (192.168.1.0/24), range (192.168.1.1-100), or list (192.168.1.1,192.168.1.2)")]
    pub targets: String,
...
fn parse_target_list(spec: &str) -> Result<Vec<String>, ModuleError> {
    ...
    for part in spec.split(',') {
        ...
        if let Some((base, cidr)) = part.split_once('/') {
            let prefix: u8 = cidr.parse().map_err(|_| ModuleError::Parse(format!("bad CIDR: {cidr}")))?;
            let base_ip: std::net::Ipv4Addr = base.parse().map_err(|_| ModuleError::Parse(format!("bad IP: {base}")))?;
            ...
            let lo_octet: u8 = lo.rsplit_once('.').map(|(_, o)| o).unwrap_or("").parse().map_err(|_| ModuleError::Parse("bad range".into()))?;
            let hi_octet: u8 = hi.trim().parse().map_err(|_| ModuleError::Parse("bad range".into()))?;
...
impl Module for ArpScanner {
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let timeout = if self.timeout_secs > 0 { self.timeout_secs } else { 2 };
        ...
        let finding = if live_hosts.is_empty() {
            ...
            format!("Found {} live host(s): {}", live_hosts.len(), live_hosts.join(", "))
        };
        ...
        if data.len() > bcc_start + 1 {
            ...
            let version = if wct == 1 {
                ...
                if data.len() >= 33 + wct * 2 + 2 + 4 {
                    let dialect_rev = u16::from_le_bytes([data[bcc_start + 2 + 32], data[bcc_start + 2 + 33]]);
                ...
                if data.len() > caps_start + 3 {
                    let caps = u32::from_le_bytes([data[caps_start], data[caps_start+1], data[caps_start+2], data[caps_start+3]]);
                    Some(format!("SMBv1 (security=0x{sec_mode:02x}, caps=0x{caps:08x})"))
                } else {
...
impl Module for FtpScanner {
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        async fn read_response(s: &tokio::net::TcpStream, t: std::time::Duration) -> Result<String, ModuleError> {
            let mut buf = vec![0u8; 4096];
        ...
        async fn send_command(s: &tokio::net::TcpStream, cmd: &[u8], t: std::time::Duration) -> Result<(), ModuleError> {
            match tokio::time::timeout(t, s.writable()).await {
                Ok(Ok(_)) => { s.try_write(cmd).map_err(|e| ModuleError::Other(format!("write: {e}")))?; Ok(()) }
        ...
        let finding = if anonymous_success {
            format!("FTP on tcp/{port}: {}  -  anonymous login enabled!", banner_short)
        } else {
...
const SSH_COMMON_CREDS: &[(&str, &str)] = &[
    ...
    ("root", "1234"), ("root", "1"), ("root", "!"), ("root", "changeme"),
    ...
    ("ubuntu", "ubuntu"), ("debian", "debian"), ("oracle", "oracle"),
    ("postgres", "postgres"), ("nagios", "nagios"), ("jenkins", "jenkins"),
];
...
pub struct SshBruteforce {
    ...
    #[option(help = "SSH port (default 22)")]
    pub port: u16,
    #[option(help = "Credentials in user:pass format, comma-separated (default: built-in wordlist)")]
    pub wordlist: String,
...
fn parse_creds(wordlist: &str) -> Vec<(String, String)> {
    if wordlist.is_empty() {
        return SSH_COMMON_CREDS.iter().map(|(u, p)| (u.to_string(), p.to_string())).collect();
    }
    wordlist.split(',').filter_map(|pair| {
        let pair = pair.trim();
...
impl Module for SshBruteforce {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&SshBruteforceOptions {
            ...
            wordlist: self.wordlist.clone(), timeout_secs: self.timeout_secs,
            concurrency: self.concurrency,
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        self.wordlist = o.wordlist; self.timeout_secs = o.timeout_secs;
        self.concurrency = o.concurrency;
        Ok(())
    }
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        let port = if self.port > 0 { self.port } else { 22 };
        let timeout_secs = if self.timeout_secs > 0 { self.timeout_secs } else { 5 };
        let max_concurrency = if self.concurrency > 0 { self.concurrency } else { 10 };
        ...
        let evidence: Vec<String> = found.iter().map(|c| {
        ...
        let finding = if found.is_empty() {
            format!("SSH bruteforce on tcp/{port}: no valid credentials found (tried {} pairs)", creds.len())
        } else {
            let details: Vec<String> = found.iter().map(|c| {
                let u = c["username"].as_str().unwrap_or("");
                let p = c["password"].as_str().unwrap_or("");
                format!("{u}:{p}")
            }).collect();
            format!("SSH bruteforce on tcp/{port}: FOUND {} valid credential(s): {}", found.len(), details.join(", "))
        };
...
impl Module for FtpBruteforce {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&FtpBruteforceOptions {
            host: self.host.clone(), port: self.port,
            wordlist: self.wordlist.clone(), timeout_ms: self.timeout_ms,
            concurrency: self.concurrency,
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.host = o.host; self.port = o.port;
        self.wordlist = o.wordlist; self.timeout_ms = o.timeout_ms;
        self.concurrency = o.concurrency;
        Ok(())
    }
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        async fn try_ftp_login(host: &str, port: u16, user: &str, pass: &str, timeout: std::time::Duration) -> Option<(String, String)> {
            let addr = format!("{host}:{port}");
            let stream = tokio::time::timeout(timeout, tokio::net::TcpStream::connect(&addr)).await.ok()?.ok()?;
            let mut buf = vec![0u8; 4096];
            let n = tokio::time::timeout(timeout, stream.readable()).await.ok()?
                .map(|_| stream.try_read(&mut buf).unwrap_or(0)).unwrap_or(0);
            if n == 0 { return None; }
            let _ = stream.try_write(format!("USER {user}\r\n").as_bytes()).ok()?;
            tokio::time::sleep(std::time::Duration::from_millis(200)).await;
            let mut buf2 = vec![0u8; 4096];
            let n2 = tokio::time::timeout(timeout, stream.readable()).await.ok()?
                .map(|_| stream.try_read(&mut buf2).unwrap_or(0)).unwrap_or(0);
            let user_resp = String::from_utf8_lossy(&buf2[..n2.min(4000)]);
            if !user_resp.starts_with("331") && !user_resp.starts_with("230") {
                return None;
            }
            let _ = stream.try_write(format!("PASS {pass}\r\n").as_bytes()).ok()?;
            tokio::time::sleep(std::time::Duration::from_millis(200)).await;
            let mut buf3 = vec![0u8; 4096];
            let n3 = tokio::time::timeout(timeout, stream.readable()).await.ok()?
                .map(|_| stream.try_read(&mut buf3).unwrap_or(0)).unwrap_or(0);
        ...
        let evidence: Vec<String> = found.iter().map(|(u, p)| format!("ftp/cred:{u}:{p}")).collect();
        let finding = if found.is_empty() {
            format!("FTP bruteforce on tcp/{port}: no valid credentials found (tried {} pairs)", creds.len())
        } else {
            let details: Vec<String> = found.iter().map(|(u, p)| format!("{u}:{p}")).collect();
            format!("FTP bruteforce on tcp/{port}: FOUND {} valid credential(s): {}", found.len(), details.join(", "))
        };
...
fn rdp_parse_neg_rsp(data: &[u8]) -> Option<(u8, u32)> {
    ...
    if data[0] != 0x02 && data[0] != 0x03 { return None; }
    let r#type = data[0];
    let _flags = data[1];
...
impl Module for RdpScanner {
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let finding = format!("RDP on tcp/{port}: selected={proto_name} (0x{selected:x}), type=0x{rsp_type:x}");
...
impl Module for VncScanner {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&VncScannerOptions {
            host: self.host.clone(), port: self.port,
            timeout_ms: self.timeout_ms, check_vnc_auth: self.check_vnc_auth,
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.host = o.host; self.port = o.port;
        self.timeout_ms = o.timeout_ms; self.check_vnc_auth = o.check_vnc_auth;
        Ok(())
    }
            ...
            if n2 >= 1 {
                ...
                if sec_count > 0 && n2 > sec_count {
                    ...
                    if n2 >= 4 {
                        ...
                        let name = match auth_type { 1 => "None", 2 => "VNC Auth", _ => "unknown" };
...
impl Module for TelnetScanner {
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let banner = clean_text.lines()
            .find(|l| !l.trim().is_empty())
            .unwrap_or("")
            .trim()
            .to_string();

        let os_hint = if banner.contains("Linux") || banner.contains("ubuntu") || banner.contains("debian") || banner.contains("centos") {
            Some("Linux")
        } else if banner.contains("FreeBSD") || banner.contains("NetBSD") || banner.contains("OpenBSD") {
            Some("BSD")
        ...
        Ok(ModuleResult {
            success: true,
            finding: Some(format!("Telnet on tcp/{port}: {banner}{}", os_hint.map(|o| format!(" [{o}]")).unwrap_or_default())),
            evidence,
            data: serde_json::json!({
                "host": self.host, "port": port,
                "banner": banner,
...
const REDIS_DEFAULT_PASSWORDS: &[&str] = &["", "redis", "default", "redislabs", "admin", "password"];
...
async fn redis_read_line(stream: &tokio::net::TcpStream, timeout: std::time::Duration) -> Result<String, ModuleError> {
    let mut buf = vec![0u8; 4096];
    let n = match tokio::time::timeout(timeout, stream.readable()).await {
        Ok(Ok(_)) => stream.try_read(&mut buf).unwrap_or(0),
        _ => return Err(ModuleError::Other("read timeout".into())),
...
async fn redis_send(stream: &tokio::net::TcpStream, cmd: &[u8], timeout: std::time::Duration) -> Result<(), ModuleError> {
    match tokio::time::timeout(timeout, stream.writable()).await {
        Ok(Ok(_)) => stream.try_write(cmd).map_err(|e| ModuleError::Other(format!("write: {e}"))).map(|_| ()),
        _ => Err(ModuleError::Other("write timeout".into())),
    }
}
