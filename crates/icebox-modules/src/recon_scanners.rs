...
use icebox_core::module::{Module, ModuleError, ModuleResult};
...
fn mysql_parse_greeting(data: &[u8]) -> Option<(String, Vec<u8>, u8, Vec<u8>)> {
    ...
    let payload = if data.len() > 4 && (data[0] as usize + 1 + data[1] as usize * 256 + 1) <= data.len() {
    ...
    let _conn_id = u32::from_le_bytes([payload[pos], payload[pos+1], payload[pos+2], payload[pos+3]]);
    pos += 4;
    ...
    pos += 1;
    pos += 10;
    let part2_len = if auth_plugin_len > 8 { auth_plugin_len as usize - 8 } else { 12 };
...
fn mysql_parse_response(data: &[u8]) -> Result<serde_json::Value, String> {
    if data.len() < 4 { return Err("response too short".into()); }
    let payload = &data[4..];
    if payload.is_empty() { return Err("empty payload".into()); }
    match payload[0] {
        0x00 => Ok(serde_json::json!({"status": "ok", "affected_rows": payload.get(1).copied().unwrap_or(0)})),
        0xFF => {
...
impl Module for MysqlScanner {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&MysqlScannerOptions {
            ...
            timeout_ms: self.timeout_ms, check_defaults: self.check_defaults,
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.host = o.host; self.port = o.port;
        self.timeout_ms = o.timeout_ms; self.check_defaults = o.check_defaults;
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let timeout = std::time::Duration::from_millis(if self.timeout_ms > 0 { self.timeout_ms } else { 5000 });
        ...
        let creds_to_try: Vec<(&str, &str)> = if !self.wordlist.is_empty() {
            self.wordlist.split(',').filter_map(|pair| {
                let pair = pair.trim();
                if let Some((u, p)) = pair.split_once(':') {
                    let u = u.trim(); let p = p.trim();
                    if !u.is_empty() { Some((u, p)) } else { None }
                } else { None }
            }).collect()
        ...
        for (user, pass) in &creds_to_try {
            ...
            if tokio::time::timeout(timeout, stream.writable()).await.is_ok() {
        ...
        let finding = if found_creds.is_empty() {
            format!("MySQL on tcp/{port}: {version} (no valid credentials found, tried {} pair(s))", creds_to_try.len())
        } else {
            let labels: Vec<String> = found_creds.iter().map(|(u, p)| {
                if p.is_empty() { format!("{u}:\"\"") } else { format!("{u}:{p}") }
            }).collect();
            format!("MySQL on tcp/{port}: {version}  -  {} credential(s) work! {}", found_creds.len(), labels.join(", "))
        };
...
const DEFAULT_WORDLIST: &[&str] = &[
    "admin", "login", "wp-admin", "wp-content", "backup", ".git", ".env", "config",
    "robots.txt", "sitemap.xml", "api", "test", "dev", "uploads", "images", "css",
    "js", "includes", "assets", "private", "restricted", "bak", "old", "src", "data",
    "console", "dashboard", "manager", "phpmyadmin", "adminer", "setup", "install",
    "readme", "changelog", "LICENSE", ".htaccess", ".htpasswd", "server-status",
    "crossdomain.xml", "clientaccesspolicy.xml", "web.config", "phpinfo.php",
    "info.php", "status", "health", "healthcheck", "actuator", "swagger",
    "api-docs", "openapi.json", "graphql", "v2", "v1", "api/v1", "api/v2",
    "ws", "websocket", "sockjs", "sockjs-node", ".well-known",
];
...
pub struct WebPathScanner {
    #[option(required = true, help = "Target base URL (e.g. http://10.0.0.1:80 or http://example.com)")]
    pub target: String,
...
fn parse_url(target: &str) -> Result<(String, u16, bool), ModuleError> {
    ...
    if let Some((host, ps)) = rest.rsplit_once(':') {
        let p: u16 = ps.parse().map_err(|_| ModuleError::Parse("bad port".into()))?;
...
impl Module for WebPathScanner {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&WebPathScannerOptions {
            target: self.target.clone(), wordlist: self.wordlist.clone(),
            timeout_ms: self.timeout_ms, concurrency: self.concurrency,
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.target = o.target; self.wordlist = o.wordlist;
        self.timeout_ms = o.timeout_ms; self.concurrency = o.concurrency;
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let timeout = std::time::Duration::from_millis(if self.timeout_ms > 0 { self.timeout_ms } else { 3000 });
        let max_concurrency = if self.concurrency > 0 { self.concurrency } else { 20 };

        let paths: Vec<String> = if self.wordlist.is_empty() {
            DEFAULT_WORDLIST.iter().map(|s| s.to_string()).collect()
        } else {
            self.wordlist.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect()
        };

        let filter_codes: Vec<u16> = if self.filter_codes.is_empty() {
            vec![200, 301, 302, 401, 403]
        } else {
            self.filter_codes.split(',')
        ...
        let finding = if discovered.is_empty() {
            ...
            let paths_str: Vec<String> = discovered.iter().map(|d| {
                let p = d["path"].as_str().unwrap_or("");
                let s = d["status"].as_u64().unwrap_or(0);
                format!("{p} [{s}]")
            }).collect();
            format!("Discovered {} path(s): {}", discovered.len(), paths_str.join(", "))
        };

        Ok(ModuleResult {
            success: true,
            finding: Some(finding),
            evidence: discovered.iter().map(|d| {
                let p = d["path"].as_str().unwrap_or("");
                let s = d["status"].as_u64().unwrap_or(0);
                format!("http{s}{p}")
            }).collect(),
...
fn dns_parse_response(data: &[u8]) -> Result<Vec<serde_json::Value>, ModuleError> {
    ...
    if rcode != 0 {
        let rcode_names = ["NOERROR", "FORMERR", "SERVFAIL", "NXDOMAIN", "NOTIMP", "REFUSED"];
        let name = rcode_names.get(rcode as usize).unwrap_or(&"UNKNOWN");
        return Err(ModuleError::Other(format!("DNS error: {name} (rcode={rcode})")));
    }
    ...
    for _ in 0..ancount {
        ...
        let rdata = match rtype {
            1 => { // A
                if rdlength >= 4 {
                    Some(serde_json::Value::String(format!("{}.{}.{}.{}", data[pos], data[pos+1], data[pos+2], data[pos+3])))
                } else { None }
            }
            28 => { // AAAA
                if rdlength >= 16 {
                    let ip = std::net::Ipv6Addr::from(<[u8; 16]>::try_from(&data[pos..pos+16]).unwrap_or([0u8; 16]));
                    Some(serde_json::Value::String(ip.to_string()))
                } else { None }
            }
            5 => { // CNAME
                let mut p = pos;
                dns_decode_name(data, &mut p).ok().map(serde_json::Value::String)
            }
            2 => { // NS
                let mut p = pos;
                dns_decode_name(data, &mut p).ok().map(serde_json::Value::String)
            ...
            16 => { // TXT
                if rdlength > 0 {
                    ...
                    let txt = String::from_utf8_lossy(&data[pos+1..pos+1+txt_len.min(rdlength-1)]).to_string();
            ...
            33 => { // SRV
                ...
                if p + 6 <= data.len() {
                    ...
                    p += 6;
                    let target = dns_decode_name(data, &mut p).unwrap_or_default();
                    Some(serde_json::json!({"priority": priority, "weight": weight, "port": port, "target": target}))
                } else { None }
            }
            _ => None,
        };

        let type_names = ["","A","NS","MD","MF","CNAME","SOA","MB","MG","MR","NULL","WKS","PTR","HINFO","MINFO","MX","TXT"];
        let type_name = if (rtype as usize) < type_names.len() { type_names[rtype as usize] } else { "TYPE?" };
...
impl Module for DnsZoneTransfer {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&DnsZoneTransferOptions {
            domain: self.domain.clone(), server: self.server.clone(),
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.domain = o.domain; self.server = o.server;
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let timeout = std::time::Duration::from_millis(if self.timeout_ms > 0 { self.timeout_ms } else { 10000 });
        ...
        for ns in &nameservers {
            ...
            let stream = match tokio::time::timeout(timeout, tokio::net::TcpStream::connect(&addr)).await {
                Ok(Ok(s)) => s,
                Ok(Err(e)) => { errors.push(format!("{ns}: connect error: {e}")); continue; }
                Err(_) => { errors.push(format!("{ns}: connect timeout")); continue; }
            };

            let query = dns_build_axfr_query(&domain, 0x1337);
            if tokio::time::timeout(timeout, stream.writable()).await.is_err() {
                errors.push(format!("{ns}: write timeout")); continue;
            }
            ...
            let n = match tokio::time::timeout(timeout, stream.readable()).await {
                Ok(Ok(_)) => stream.try_read(&mut buf).unwrap_or(0),
                _ => { errors.push(format!("{ns}: read timeout")); continue; }
            };

            if n < 4 {
                errors.push(format!("{ns}: response too short ({n} bytes)")); continue;
            }
        ...
        let evidence: Vec<String> = all_records.iter().map(|r| {
        ...
        let ns_used_str = all_records.first()
            .and_then(|_| nameservers.first())
            .unwrap_or(&"none");

        let finding = if all_records.is_empty() {
            format!("DNS zone transfer failed for {domain}: {}", errors.join("; "))
        } else {
...
fn whois_server_for(query: &str) -> &'static str {
    ...
    if q.contains('.') {
        if q.ends_with(".edu") { return "whois.educause.edu"; }
        if q.ends_with(".gov") { return "whois.nic.gov"; }
        if q.ends_with(".mil") { return "whois.nic.mil"; }
        if q.ends_with(".org") || q.ends_with(".ngo") || q.ends_with(".ong") {
            return "whois.publicinterestregistry.net";
        }
        if q.ends_with(".info") { return "whois.afilias.net"; }
        if q.ends_with(".biz") { return "whois.nic.biz"; }
        if q.ends_with(".io") { return "whois.nic.io"; }
        if q.ends_with(".co") { return "whois.nic.co"; }
        if q.ends_with(".int") { return "whois.iana.org"; }
...
impl Module for WhoisLookup {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&WhoisLookupOptions {
            query: self.query.clone(), server: self.server.clone(),
            timeout_ms: self.timeout_ms,
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.query = o.query; self.server = o.server;
        self.timeout_ms = o.timeout_ms;
        Ok(())
    }
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let timeout = std::time::Duration::from_millis(if self.timeout_ms > 0 { self.timeout_ms } else { 15000 });
        ...
        match tokio::time::timeout(timeout, stream.writable()).await {
            Ok(Ok(_)) => { stream.try_write(request.as_bytes()).map_err(|e| ModuleError::Other(format!("write: {e}")))?; }
        ...
        loop {
            ...
            if all_data.len() > 65536 { break; }
        }

        let raw_text = String::from_utf8_lossy(&all_data).to_string();
        let summary_lines: Vec<&str> = raw_text.lines()
            .filter(|l| {
                let l = l.trim().to_lowercase();
                l.starts_with("domain") || l.starts_with("registrar") ||
                l.starts_with("creation") || l.starts_with("expir") ||
                l.starts_with("name server") || l.starts_with("nserver") ||
                l.starts_with("org") || l.starts_with("netrange") ||
                l.starts_with("cidr") || l.starts_with("orgname")
            })
            .collect();
        ...
        Ok(ModuleResult {
            ...
            finding: Some(format!("Whois lookup for {query} via {server}: {} lines", raw_text.lines().count())),
...
const DEFAULT_SUBDOMAINS: &[&str] = &[
    "www", "mail", "admin", "ftp", "ssh", "api", "dev", "test", "blog", "shop",
    "portal", "remote", "vpn", "webmail", "dns", "ns1", "ns2", "smtp", "pop3",
    "imap", "owa", "cpanel", "whm", "phpmyadmin", "jenkins", "gitlab", "jira",
    "wiki", "docs", "status", "support", "chat", "demo", "stage", "staging",
    "backup", "monitor", "logs", "db", "data", "sql", "mysql", "redis", "mongo",
    "cdn", "assets", "static", "download", "upload", "app", "apps", "server",
    "crm", "erp", "intranet", "biz", "info", "partner", "partners", "client",
    "clients", "user", "users", "account", "accounts", "login", "signin",
    "auth", "sso", "identity", "api-v1", "api-v2", "v1", "v2", "graphql",
    "rest", "service", "services", "mq", "kafka", "rabbitmq", "elasticsearch",
    "kibana", "grafana", "prometheus", "nagios", "zabbix", "mx", "mail2",
    "mail1", "ns", "ns3", "ns4", "direct", "direct-connect", "web", "webmail2",
    "forum", "help", "helpdesk", "knowledgebase", "kb", "tickets", "git",
    "svn", "cloud", "office", "outlook", "exchange", "lync", "skype",
];
...
impl Module for SubdomainEnum {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&SubdomainEnumOptions {
            domain: self.domain.clone(), wordlist: self.wordlist.clone(),
            timeout_ms: self.timeout_ms, concurrency: self.concurrency,
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.domain = o.domain; self.wordlist = o.wordlist;
        self.timeout_ms = o.timeout_ms; self.concurrency = o.concurrency;
        Ok(())
    }
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        let domain = self.domain.trim().to_lowercase();
        let timeout = std::time::Duration::from_millis(if self.timeout_ms > 0 { self.timeout_ms } else { 3000 });
        let max_concurrency = if self.concurrency > 0 { self.concurrency } else { 50 };
        ...
        for sub in &subdomains {
            ...
            handles.push(tokio::spawn(async move {
                ...
                let result = tokio::time::timeout(timeout, tokio::net::lookup_host((fqdn.clone(), 0))).await;
                match result {
                    Ok(Ok(addrs)) => {
        ...
        resolved.sort_by(|a, b| {
            a["subdomain"].as_str().unwrap_or("").cmp(b["subdomain"].as_str().unwrap_or(""))
        });

        let evidence: Vec<String> = resolved.iter().map(|r| {
            let f = r["fqdn"].as_str().unwrap_or("");
            let ips = r["ips"].as_array().map(|a| a.iter()
        ...
        let finding = if resolved.is_empty() {
            ...
            let list: Vec<String> = resolved.iter().map(|r| {
                let s = r["subdomain"].as_str().unwrap_or("");
                let ips = r["ips"].as_array()
...
fn ber_oid(components: &[u32]) -> Vec<u8> {
    ...
    if components.len() < 2 { return ber_tag(0x06, &[]); }
...
fn asn1_skip_tlv(data: &[u8], pos: &mut usize) -> Result<(), ModuleError> {
    ...
    *pos += 1;
    let length: usize = if len_byte < 0x80 {
        ...
        if *pos >= data.len() { return Err(ModuleError::Other("ASN.1: EOF in long length".into())); }
        let l = data[*pos] as usize;
        *pos += 1;
        l
    } else if len_byte == 0x82 {
        if *pos + 2 > data.len() { return Err(ModuleError::Other("ASN.1: EOF in long length".into())); }
        let l = u16::from_be_bytes([data[*pos], data[*pos+1]]) as usize;
        *pos += 2;
        l
    } else {
        return Err(ModuleError::Other("ASN.1: unsupported length encoding".into()));
    };
    *pos += length;
    Ok(())
}
...
fn asn1_get_value(data: &[u8], pos: &mut usize) -> Result<Vec<u8>, ModuleError> {
    ...
    *pos += 1;
    let length: usize = if len_byte < 0x80 {
        ...
        if *pos >= data.len() { return Err(ModuleError::Other("ASN.1: EOF".into())); }
        let l = data[*pos] as usize;
        *pos += 1;
        l
    } else if len_byte == 0x82 {
        if *pos + 2 > data.len() { return Err(ModuleError::Other("ASN.1: EOF".into())); }
...
fn snmp_parse_response(data: &[u8]) -> Result<String, ModuleError> {
    ...
    if value.iter().all(|&b| (0x20..=0x7E).contains(&b) || b == b' ') {
...
fn asn1_parse_for_text(data: &[u8]) -> Option<String> {
    ...
    let mut candidates: Vec<String> = text.split(|c: char| !c.is_ascii_graphic() && c != ' ')
...
impl Module for SnmpScanner {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&SnmpScannerOptions {
            host: self.host.clone(), port: self.port,
            timeout_ms: self.timeout_ms, communities: self.communities.clone(),
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.host = o.host; self.port = o.port;
        self.timeout_ms = o.timeout_ms; self.communities = o.communities;
        Ok(())
    }
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let communities: Vec<&str> = if self.communities.is_empty() {
            vec!["public", "private", "community", "snmp", "manager", "admin", "read", "write"]
        } else {
            self.communities.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect()
        };
        ...
        for &community in &communities {
            ...
            if tokio::time::timeout(timeout, sock.send_to(&request, &addr)).await.is_err() {
                continue;
            }
            let mut buf = vec![0u8; 65536];
        ...
        let evidence: Vec<String> = found_communities.iter()
        ...
        let finding = if found_communities.is_empty() {
            ...
            let summaries: Vec<String> = found_communities.iter()
                .map(|(c, desc)| format!("{c}={desc}"))
                .collect();
            format!("SNMP on udp/{port}: {} working community string(s): {}", found_communities.len(), summaries.join("; "))
        };
...
fn bson_encode_is_master() -> Vec<u8> {
    let mut doc = Vec::new();
    doc.push(0x10); doc.extend_from_slice(b"isMaster\x00");
    ...
    while pos + 2 < doc.len() {
        ...
        if name == field {
            return match etype {
                ...
                0x08 => {
                    if pos < doc.len() {
                        Some(if doc[pos] != 0 { "true".into() } else { "false".into() })
                    } else { None }
                }
                _ => None,
            };
        ...
        // Skip value
        pos = match etype {
            0x01 => { if pos + 8 <= doc.len() { pos + 8 } else { return None; } }
            0x02 => {
                if pos + 4 > doc.len() { return None; }
                let slen = i32::from_le_bytes([doc[pos], doc[pos+1], doc[pos+2], doc[pos+3]]) as usize;
                if pos + 4 + slen <= doc.len() { pos + 4 + slen } else { return None; }
            }
            0x03 | 0x04 => {
                if pos + 4 > doc.len() { return None; }
                let dlen = i32::from_le_bytes([doc[pos], doc[pos+1], doc[pos+2], doc[pos+3]]) as usize;
                if pos + dlen <= doc.len() { pos + dlen } else { return None; }
            }
            0x08 => pos + 1,
            0x09 | 0x12 => { if pos + 8 <= doc.len() { pos + 8 } else { return None; } }
...
fn bson_find_int32(doc: &[u8], field: &str) -> Option<i32> {
    ...
    while pos + 2 < doc.len() {
        ...
        pos = match etype {
            0x01 => pos + 8, 0x02 => { if pos + 4 > doc.len() { return None; } pos + 4 + i32::from_le_bytes([doc[pos], doc[pos+1], doc[pos+2], doc[pos+3]]) as usize }
            0x03 | 0x04 => { if pos + 4 > doc.len() { return None; } pos + i32::from_le_bytes([doc[pos], doc[pos+1], doc[pos+2], doc[pos+3]]) as usize }
            0x08 => pos + 1, 0x09 | 0x12 => pos + 8, 0x10 => pos + 4,
            _ => { return None; }
        };
    }
    None
...
fn mongo_body(response: &[u8]) -> Option<&[u8]> {
    if response.len() < 16 { return None; }
    ...
    if op_code != 1 { return None; } // OP_REPLY
    if response.len() < 36 { return None; }
    let _flags = i32::from_le_bytes([response[16], response[17], response[18], response[19]]);
    let _cursor = i64::from_le_bytes([response[20], response[21], response[22], response[23],
...
impl Module for MongoScanner {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&MongoScannerOptions {
            host: self.host.clone(), port: self.port, timeout_ms: self.timeout_ms,
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.host = o.host; self.port = o.port; self.timeout_ms = o.timeout_ms;
        Ok(())
    }
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let timeout_ms = if self.timeout_ms > 0 { self.timeout_ms } else { 5000 };
...
async fn es_http_request(host: &str, port: u16, path: &str, timeout: std::time::Duration) -> Result<Vec<u8>, ModuleError> {
    let addr = format!("{host}:{port}");
    let stream = match tokio::time::timeout(timeout, tokio::net::TcpStream::connect(&addr)).await {
        Ok(Ok(s)) => s,
...
impl Module for EsScanner {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&EsScannerOptions {
            host: self.host.clone(), port: self.port,
            timeout_ms: self.timeout_ms, check_health: self.check_health,
        }).unwrap_or(serde_json::Value::Null)
    }
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.host = o.host; self.port = o.port;
        self.timeout_ms = o.timeout_ms; self.check_health = o.check_health;
        Ok(())
    }
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        if self.check_health {
            if let Ok(health_raw) = es_http_request(&self.host, port, "/_cluster/health", timeout).await {
...
const PG_DEFAULT_CREDS: &[(&str, &str)] = &[
    ("postgres", "postgres"), ("postgres", "admin"), ("postgres", "password"),
...
fn pg_auth_type_name(t: i32) -> &'static str {
    match t { 0 => "OK", 2 => "KerberosV5", 3 => "CleartextPassword", 5 => "MD5Password", 6 => "SCMCredential", 7 => "GSS", 9 => "SSPI", 10 => "SASL", 11 => "SASLContinue", 12 => "SASLFinal", _ => "Unknown" }
}
...
impl Module for PostgresScanner {
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        async fn pg_read_response(stream: &tokio::net::TcpStream, timeout: std::time::Duration) -> Result<Vec<u8>, ModuleError> {
            let mut buf = vec![0u8; 8192];
            let n = match tokio::time::timeout(timeout, stream.readable()).await {
                Ok(Ok(_)) => stream.try_read(&mut buf).unwrap_or(0),
        ...
        for (user, pass) in &creds_to_try {
            ...
            if tokio::time::timeout(timeout, stream.writable()).await.is_err() { continue; }
            ...
            if response[0] == b'R' {
                if response.len() < 9 { continue; }
                let auth_t = i32::from_be_bytes([response[5], response[6], response[7], response[8]]);
                ...
                if auth_t == 0 {
                        ...
                        if n > 0 {
                            ...
                            for line in text.split('\x00') {
                                if line.starts_with("server_version") {
                                    server_version = line.trim_start_matches("server_version").trim_start_matches('\x00').to_string();
                                }
                            }
                        }
                    }
                    ...
                    if resp2.len() > 5 && resp2[0] == b'R' {
                        ...
                        if auth_t2 == 0 { found_creds.push((user.to_string(), pass.to_string())); }
                    }
        ...
        let evidence: Vec<String> = found_creds.iter()
            .map(|(u, p)| format!("postgres/cred:{u}:{p}"))
            .collect();

        let finding = if !found_creds.is_empty() {
            let labels: Vec<String> = found_creds.iter().map(|(u, p)| format!("{u}:{p}")).collect();
            format!("PostgreSQL on tcp/{port}: FOUND {} credential(s): {}", found_creds.len(), labels.join(", "))
        } else {
...
async fn smtp_read(stream: &tokio::net::TcpStream, timeout: std::time::Duration) -> String {
    ...
    for _ in 0..3 {
        ...
        if all.len() > 8192 { break; }
    }
    String::from_utf8_lossy(&all).to_string()
}

async fn smtp_write(stream: &tokio::net::TcpStream, cmd: &str, timeout: std::time::Duration) -> Result<(), ModuleError> {
    match tokio::time::timeout(timeout, stream.writable()).await {
        Ok(Ok(_)) => stream.try_write(cmd.as_bytes()).map_err(|e| ModuleError::Other(format!("write: {e}"))).map(|_| ()),
...
impl Module for SmtpScanner {
    fn options_json(&self) -> serde_json::Value {
        serde_json::to_value(&SmtpScannerOptions {
            host: self.host.clone(), port: self.port,
            timeout_ms: self.timeout_ms, check_relay: self.check_relay,
    ...
    fn set_option(&mut self, name: &str, value: &str) -> Result<(), ModuleError> {
        ...
        o.set(name, value)?;
        self.host = o.host; self.port = o.port;
        self.timeout_ms = o.timeout_ms; self.check_relay = o.check_relay;
    ...
    async fn run(&self) -> Result<ModuleResult, ModuleError> {
        ...
        let ehlo_ok = ehlo_lines.first().map(|l| l.starts_with("250")).unwrap_or(false);

        if ehlo_ok {
            let caps: Vec<String> = ehlo_lines.iter()
                .filter_map(|l| {
