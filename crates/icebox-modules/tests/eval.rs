...
use icebox_core::module::{Capability, Intent, LoadedModule};
...
fn decide(exec: &ModuleExecutor, loaded: &LoadedModule, target: &str, approved: bool) -> PolicyDecision {
...
fn capability_and_intent_model() {
    let loaded = icebox_modules::load("ssh_bruteforce").expect("module present");
    assert!(loaded.info.capabilities.contains(&Capability::CredentialAccess));
    ...
    let scan = icebox_modules::load("tcp_port_scanner").expect("module present");
    assert_eq!(scan.info.effective_impact(), RiskLevel::Low);
}

#[tokio::test]
async fn safety_gates_block_before_run() {
    let loaded = icebox_modules::load("reverse_shell_payload").expect("module present");
    let exec = executor(RiskLevel::Critical, &["10.0.0.5"]);

    assert!(
        matches!(decide(&exec, &loaded, "8.8.8.8", true), PolicyDecision::Deny(_)),
        "out-of-scope target must be denied"
    );
    assert!(
        matches!(decide(&exec, &loaded, "10.0.0.5", false), PolicyDecision::RequireApproval(_)),
        "high-risk payload needs approval"
    );
    assert!(
        matches!(decide(&exec, &loaded, "10.0.0.5", true), PolicyDecision::Allow),
...
async fn operator_rules_override() {
    ...
    assert!(
        matches!(decide(&exec, &loaded, "10.0.0.5", true), PolicyDecision::Deny(_)),
        "deny rule wins over approval"
    );

    let mut exec2 = executor(RiskLevel::Critical, &["10.0.0.5"]);
    exec2.policy_set.add_rule(PolicyRule::MaxRisk(RiskLevel::Low));
    assert!(
        matches!(decide(&exec2, &loaded, "10.0.0.5", true), PolicyDecision::Deny(_)),
    ...
    assert!(
        matches!(decide(&exec3, &loaded, "10.0.0.5", false), PolicyDecision::Allow),
