...
impl PlanApprover for InteractiveApprover {
    fn approve(&self, actions: &[Action]) -> bool {
        ...
        for (i, a) in sorted.iter().enumerate() {
            let opts: Vec<String> = a.options.iter()
                .map(|(k, v)| format!("{k}={v}"))
                .collect();
            println!(
                "  {}. [prio {}] {} -> {} | {} | opts: [{}]",
                i + 1, a.priority, a.module, a.target, a.reason, opts.join(", ")
            );
        }
...
pub fn build_module_catalog() -> String {
    let entries = icebox_modules::discover();
    let mut lines = Vec::new();
    for e in entries {
        let info = (e.info)();
        let loaded = match icebox_modules::load(&info.name) {
            Some(l) => l,
            None => continue,
        };
        let opt_desc = match loaded.module.options_json() {
            serde_json::Value::Object(map) if !map.is_empty() => {
                map.iter().map(|(k, v)| {
                    let t = if v.is_u64() || v.is_i64() { "int" }
        ...
        lines.push(format!(
            "- {} [{}]: {}. options: [{}]",
            info.name, info.kind.as_str(), info.description, opt_desc
        ));
    }
    lines.join("\n")
}
...
impl Planner for LlmPlanner {
    async fn analyze(&self, context: &str) -> anyhow::Result<AnalysisOutput> {
        ...
        let reply = self
            .llm
            .prompt(&self.system_prompt(), &prompt, Some(Self::analysis_schema()))
            .await?;
        let cleaned = clean_json(&reply);
        match serde_json::from_str::<AnalysisOutput>(&cleaned) {
            Ok(a) => Ok(a),
            Err(_) => {
                let raw = self.llm.prompt(&self.system_prompt(), &prompt, None).await?;
...
pub struct Agent {
    ...
    job_ids: Vec<icebox_core::job::JobId>,
...
impl Agent {
    ...
    pub async fn run(&mut self) -> anyhow::Result<CampaignResult> {
        ...
        loop {
            ...
            if !proceed {
                self.logs.push("plan rejected by operator; stopping campaign".into());
                break;
            }
            if actions.is_empty() {
                self.logs.push("plan: no actions generated; stopping".into());
                break;
            }
            ...
            if iteration >= self.max_iterations {
                self.logs.push(format!("reached max iterations ({})", self.max_iterations));
                break;
            }
            if produced == 0 {
                self.logs.push("no new results this iteration; stopping".into());
                break;
            }
    ...
    async fn run_scan(&mut self) -> anyhow::Result<()> {
        ...
        let scanners: Vec<_> = entries
            ...
            .filter(|e| {
                ...
                matches!(kind, icebox_core::module::ModuleKind::Scanner | icebox_core::module::ModuleKind::Auxiliary)
            })
            .collect();

        if scanners.is_empty() {
            self.logs
                .push(format!("scan: no scanner modules, available: {}", names.join(", ")));
            return Ok(());
        }

        for entry in scanners {
            let info = (entry.info)();
            let mut loaded = icebox_modules::load(&info.name).unwrap();
            ...
            if info.name == "service_fingerprinter" {
                let _ = loaded.module.set_option("host", &self.target);
                let _ = loaded.module.set_option("ports", "22,80,443,3306,5432,6379,8080,8443");
            }
            let mut fw = self.fw.lock().await;
            let pf = fw
                .executor
                .preflight(&loaded, &self.target, None, self.approved, PolicyContext::Autonomous);
            ...
            match fw
                .executor
                .execute(&loaded, &self.target, None, self.approved, PolicyContext::Autonomous, Some(jid.as_u64()))
                .await
            {
                Ok(r) => {
    ...
    async fn run_analyze(&mut self) -> anyhow::Result<()> {
        ...
        self.logs
            .push(format!("analysis: {}  -  vulns: {}", analysis.summary, analysis.vulnerabilities.join(", ")));
        let mut fw = self.fw.lock().await;
    ...
    async fn run_execute(&mut self, actions: Vec<Action>) -> anyhow::Result<()> {
        for action in actions {
            let Some(loaded) = icebox_modules::load(&action.module) else {
                self.logs.push(format!("execute: module {} not found", action.module));
                continue;
            };
            self.logs
                .push(format!("execute: {} -> {} ({})", action.module, action.target, action.reason));
            ...
            let pf = fw
                .executor
                .preflight(&loaded_mut, &self.target, None, self.approved, PolicyContext::Autonomous);
            let policy = make_config_policy(
                self.max_risk,
                PolicyContext::Autonomous,
                &fw.executor.policy_set,
            ...
            match fw
                .executor
                .execute(&loaded_mut, &self.target, None, self.approved, PolicyContext::Autonomous, Some(jid.as_u64()))
                .await
            {
                Ok(r) => {
                    fw.jobs.complete(jid, r.clone());
                    ...
                    fw.executor
                        .remember(MemoryKind::Decision, format!("executed {}: {}", action.module, outcome));
                ...
                Err(e) => {
                    fw.jobs.cancel(jid);
                    fw.executor
                        .remember(MemoryKind::Failure, format!("{} failed: {e}", action.module));
                    self.logs.push(format!("execute: {} error: {e}", action.module));
                }
            }
        }
        let modules: Vec<String> = self.executed_results.iter().map(|r| r.module.clone()).collect();
        let mut fw = self.fw.lock().await;
        fw.executor.record_trace(ReasoningTrace {
            at: now_secs(),
            phase: "execute".into(),
    ...
    async fn build_context(&self) -> String {
        let mut ctx = format!("Target: {}\nPhase: {}\n\n", self.target, self.phase.as_str());
        ...
        let jobs: Vec<String> = fw
            ...
            .list_recent(20)
            .iter()
            .map(|j| format!("job {}: {} {} -> {}", j.id, j.module_name, j.target, j.status.as_str()))
            .collect();
        ...
        if !memories.is_empty() {
            ctx.push_str(&format!("Memory (planner learnings):\n{}\n", memories.join("\n")));
        }
...
pub struct CampaignResult {
    ...
    pub job_ids: Vec<icebox_core::job::JobId>,
    pub report: String,
}
