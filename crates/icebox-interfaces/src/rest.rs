...
use icebox_core::governance::{ApprovalRequest, ApprovalStatus, PolicyPack, Role, audit_to_csv, role_allows};
...
pub async fn serve(fw: SharedFramework, addr: SocketAddr) -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route(
            "/api/v1/policy/rules",
            get(list_policy_rules).post(add_policy_rule).put(replace_policy_rules),
        )
        ...
        .route("/api/v1/policy/packs", get(list_policy_packs).post(add_policy_pack))
        .route("/api/v1/policy/pack/{name}/apply", post(apply_policy_pack))
        .route("/api/v1/approvals", get(list_approvals).post(request_approval))
...
async fn list_modules(State(_fw): State<SharedFramework>) -> Json<Vec<ModuleItem>> {
    let items = icebox_modules::discover()
        .into_iter()
        .map(|e| {
            let i = (e.info)();
            ModuleItem { name: i.name, kind: i.kind.as_str().into(), description: i.description }
        })
        .collect();
    Json(items)
}

async fn get_module(State(fw): State<SharedFramework>, Path(name): Path<String>) -> Json<Option<ModuleDetail>> {
    let fw = fw.lock().await;
    let loaded = match icebox_modules::load(&name) {
        Some(l) => l,
        None => return Json(None),
    };
    let pf = fw.executor.preflight(&loaded, "", None, false, PolicyContext::Rest);
...
async fn set_option(
    ...
    let Some(mut loaded) = icebox_modules::load(&name) else { return Json(Err("module not found".into())); };
...
async fn run_module(
    ...
    let Some(mut loaded) = icebox_modules::load(&name) else {
        return Json(RunResponse {
            job_id: 0, session_id: None, success: false, data: serde_json::Value::Null,
            preflight: None, error: Some("module not found".into()),
        });
    };
    ...
    if !role_allows(fw.operator_role, Role::Operator) {
        return Json(RunResponse {
            job_id: 0, session_id: None, success: false, data: serde_json::Value::Null,
            preflight: None, error: Some("forbidden: operator role required".into()),
        });
    }
    ...
    let result = fw
        .executor
        .execute(&loaded, &payload.target, None, payload.approved, PolicyContext::Rest, Some(job_id.as_u64()))
        .await;
    ...
    match result {
        Ok(r) => {
            ...
            if let Some(ref s) = r.session_id {
                let kind = if s.starts_with("session:") { SessionKind::Shell } else { SessionKind::Unknown };
                let new_sid = fw.sessions.register(Session::new(kind, &payload.target, &loaded.info.name));
...
async fn run_agent(
    ...
    if !role_allows(fw.lock().await.operator_role, Role::Operator) {
        return Json(AgentRunResponse {
            ...
            actions_taken: vec![], sessions_opened: vec![], job_ids: vec![], report: String::new(),
        });
    }
    ...
    if payload.approve {
        agent.set_plan_approver(Box::new(icebox_ai::agent::AlwaysApprove));
    } else {
        agent.set_plan_approver(Box::new(icebox_ai::agent::DenyPlan));
    }
...
) -> Json<icebox_ai::CampaignReport> {
    let model = payload.model.unwrap_or_else(|| "llama3.2".into());
    if !role_allows(fw.lock().await.operator_role, Role::Operator) {
        return Json(icebox_ai::CampaignReport {
            targets: payload.targets.clone(), summaries: vec!["forbidden: operator role required".into()],
            ok: 0, failed: 0, total_jobs: 0, total_sessions: 0, total_decisions: 0, total_evidence: 0, total_traces: 0,
        });
    }
    let mut orch = Orchestrator::new(fw, RiskLevel::High);
    orch.set_approved(payload.approve);
    Json(orch.run(&payload.targets, move || Box::new(LlmPlanner::new(model.clone()))).await)
}
...
async fn accept_charter(State(fw): State<SharedFramework>, Json(payload): Json<CharterPayload>) -> Json<String> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
...
async fn add_scope(State(fw): State<SharedFramework>, Json(payload): Json<ScopePayload>) -> Json<String> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
        return Json("forbidden: operator role required".into());
    }
...
async fn evaluate_policy(
    ...
    let fw = fw.lock().await;
    let Some(loaded) = icebox_modules::load(&params.module) else {
    ...
    Json(PolicyEvaluation {
        ...
        intents: pf.capabilities.iter().map(|c| c.intent().as_str().into()).collect(),
...
async fn add_policy_rule(State(fw): State<SharedFramework>, Json(rule): Json<PolicyRule>) -> Json<PolicySet> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
...
async fn replace_policy_rules(State(fw): State<SharedFramework>, Json(rules): Json<Vec<PolicyRule>>) -> Json<PolicySet> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
        return Json(fw.executor.policy_set.clone());
    }
...
async fn delete_policy_rule(State(fw): State<SharedFramework>, Path(index): Path<usize>) -> Result<Json<PolicySet>, StatusCode> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
...
async fn save_workspace(State(fw): State<SharedFramework>, Json(p): Json<WorkspacePath>) -> Result<Json<String>, StatusCode> {
    let fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
        return Err(StatusCode::FORBIDDEN);
    }
...
async fn load_workspace(State(fw): State<SharedFramework>, Json(p): Json<WorkspacePath>) -> Result<Json<String>, StatusCode> {
...
async fn export_audit(State(fw): State<SharedFramework>, Query(params): Query<AuditQuery>) -> Response {
    let fw = fw.lock().await;
...
async fn add_policy_pack(State(fw): State<SharedFramework>, Json(pack): Json<PolicyPack>) -> Result<Json<PolicyPack>, StatusCode> {
    let mut fw = fw.lock().await;
...
async fn apply_policy_pack(State(fw): State<SharedFramework>, Path(name): Path<String>) -> Result<Json<PolicySet>, StatusCode> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Admin) {
        return Err(StatusCode::FORBIDDEN);
    }
...
async fn request_approval(State(fw): State<SharedFramework>, Json(input): Json<ApprovalRequestInput>) -> Json<ApprovalRequest> {
    let mut fw = fw.lock().await;
    let id = fw.approval_queue.request(input.module, input.target, input.reason, input.options);
    Json(fw.approval_queue.get(id).cloned().unwrap())
}

async fn approve_approval(State(fw): State<SharedFramework>, Path(id): Path<u64>) -> Result<Json<String>, StatusCode> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
        return Err(StatusCode::FORBIDDEN);
    }
    ...
    match fw.executor.execute(&loaded, &req.target, None, true, PolicyContext::Rest, None).await {
        Ok(_) => Ok(Json(format!("request {id} approved and executed"))),
        Err(e) => Ok(Json(format!("request {id} approved but execution failed: {e}"))),
    }
}

async fn deny_approval(State(fw): State<SharedFramework>, Path(id): Path<u64>) -> Result<Json<String>, StatusCode> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Operator) {
        return Err(StatusCode::FORBIDDEN);
    }
...
async fn set_role(State(fw): State<SharedFramework>, Json(payload): Json<RolePayload>) -> Result<Json<Role>, StatusCode> {
    let mut fw = fw.lock().await;
    if !role_allows(fw.operator_role, Role::Admin) {
        return Err(StatusCode::FORBIDDEN);
    }
