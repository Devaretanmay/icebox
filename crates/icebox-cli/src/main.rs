...
use icebox_core::framework::{new_shared_framework, Framework, SharedFramework};
use icebox_core::governance::{PolicyPack, Role, audit_to_csv, role_allows};
use icebox_core::job::Job;
use icebox_core::module::LoadedModule;
use icebox_core::Capability;
use icebox_core::safety::{
    Charter, PolicyContext, PolicyDecision, PolicyEngine, PolicyRule, RiskLevel, ScopeManager,
};
use icebox_core::session::{Session, SessionId, SessionKind};
...
async fn main() -> anyhow::Result<()> {
    ...
    let state = Arc::new(Mutex::new(CliState { fw: fw.clone(), loaded: None, target: None }));
    ...
    let api_handle = std::thread::spawn(move || {
        ...
        rt.block_on(async { icebox_interfaces::rest::serve(fw_api, addr).await })
    });
    ...
    loop {
        ...
        if matches!(c.as_str(), "agent" | "save" | "load" | "validate" | "role" | "pack" | "approve" | "campaign") {
            drop(s);
            match c.as_str() {
...
async fn cmd_list() {
    for e in icebox_modules::discover() {
        let i = (e.info)();
...
fn cmd_use(a: &[&str], s: &mut CliState, fw: &Framework) {
    let n = a.join(" ");
    if n.is_empty() { println!("usage: use <name>"); return; }
    match icebox_modules::load(&n) {
        Some(l) => {
            let pf = fw
                .executor
                .preflight(&l, &s.target.clone().unwrap_or_default(), None, false, PolicyContext::Cli);
...
fn cmd_info(s: &CliState) {
    let Some(ref l) = s.loaded else { println!("no module"); return; };
    println!("name: {}", l.info.name);
    println!("kind: {}", l.info.kind.as_str());
    println!("desc: {}", l.info.description);
    if !l.info.author.is_empty() { println!("author: {}", l.info.author); }
    println!("options: {}", l.module.options_json());
}

fn cmd_set(a: &[&str], s: &mut CliState) {
    let Some(l) = &mut s.loaded else { println!("no module"); return; };
    if a.len() < 2 { println!("usage: set <key> <val>"); return; }
...
fn cmd_show(a: &[&str], s: &CliState) {
    if a.first().copied() != Some("options") { println!("usage: show options"); return; }
    match &s.loaded { None => println!("no module"), Some(l) => println!("{}", l.module.options_json()) }
}

fn cmd_charter(a: &[&str], fw: &mut Framework) {
    match a.first().copied().unwrap_or("") {
        "accept" => {
            if a.len() < 2 { println!("usage: charter accept <name>"); return; }
...
fn cmd_scope(a: &[&str], fw: &mut Framework) {
    match a.first().copied().unwrap_or("") {
        "add" => {
            let t = a[1..].join(" ");
            if t.is_empty() { println!("usage: scope add <tgt>"); return; }
            if !role_allows(fw.operator_role, Role::Operator) {
                println!("forbidden: operator role required");
                return;
            }
        ...
        "show" => for t in &fw.executor.scope.allow { println!("  {t}"); },
...
fn cmd_sessions(a: &[&str], fw: &mut Framework) {
    if a.first().copied() == Some("close") {
        ...
        if let Some(id) = id { if fw.sessions.close(id) { println!("session {id} closed"); return; } }
        println!("not found"); return;
    }
    for s in fw.sessions.list() {
        println!("  {}  {}  {}  ({:.0?})", s.id, s.kind.as_str(), s.target, s.elapsed());
    }
}
...
fn cmd_jobs(a: &[&str], fw: &Framework) {
    ...
    for j in fw.jobs.list_recent(n) {
        println!("  {}  {}  {}  {}  ({:.0?})", j.id, j.module_name, j.target, j.status.as_str(), j.elapsed());
    }
}
...
async fn cmd_save(a: &[&str], fw: SharedFramework) {
    ...
    let path = if a.is_empty() { "workspace.json".to_string() } else { a.join(" ") };
    let snap = {
        let fw_guard = fw.lock().await;
        icebox_core::workspace::WorkspaceSnapshot::from_framework(&fw_guard)
    };
...
async fn cmd_load(a: &[&str], fw: SharedFramework) {
    ...
    let path = if a.is_empty() { "workspace.json".to_string() } else { a.join(" ") };
    match icebox_core::workspace::WorkspaceSnapshot::load_from_file(&path) {
        Ok(snap) => {
...
async fn cmd_agent(a: &[&str], fw: SharedFramework) {
    ...
    let (model, target) = if a.get(1).copied() == Some("--model") {
        (a.get(2).unwrap_or(&"llama3.2").to_string(), a[3..].join(" "))
    } else {
    ...
    let planner = Box::new(icebox_ai::agent::LlmPlanner::new(model));
    let mut agent = icebox_ai::agent::Agent::new(planner, fw, target, RiskLevel::High);
    agent.set_plan_approver(Box::new(icebox_ai::agent::InteractiveApprover));
...
async fn cmd_campaign(a: &[&str], fw: SharedFramework) {
    ...
    println!("campaign: {} target(s) (approved: {approved})", targets.len());
    let orch = icebox_ai::Orchestrator::new(fw, RiskLevel::High);
    let report = orch
        .run(&targets, || Box::new(icebox_ai::agent::LlmPlanner::new("llama3.2")))
        .await;
    ...
    println!(
        ...
        report.total_jobs, report.total_sessions, report.total_decisions, report.total_evidence, report.total_traces
    );
}
...
async fn cmd_validate(a: &[&str], fw: SharedFramework) {
    match a.first().copied() {
        Some("diff") => {
            ...
            let ra: icebox_ai::ValidationReport = match load_report(pa) {
                Some(r) => r,
                None => return,
            };
            let rb: icebox_ai::ValidationReport = match load_report(pb) {
                Some(r) => r,
                None => return,
            };
            let d = icebox_ai::diff(&ra, &rb);
            println!(
        ...
        Some("run") | None => {
            ...
            let report = icebox_ai::run_validation(
                fw.clone(),
                &targets,
                RiskLevel::High,
                move || Box::new(icebox_ai::agent::LlmPlanner::new(model_for_closure.clone())),
            )
            .await;
            println!(
...
fn load_report(path: &str) -> Option<icebox_ai::ValidationReport> {
    match std::fs::read_to_string(path) {
        Ok(s) => match serde_json::from_str::<icebox_ai::ValidationReport>(&s) {
            Ok(r) => Some(r),
            Err(e) => {
...
async fn cmd_run(a: &[&str], s: &mut CliState, fw: &mut Framework) {
    ...
    let tp = if a.first().copied() == Some("--approve") { approved = true; &a[1..] } else { a };
    let target = if !tp.is_empty() { Some(tp.join(" ")) } else { s.target.clone() };
    let Some(target) = target else { println!("target required"); return; };
    s.target = Some(target.clone());

    let pf = fw.executor.preflight(l, &target, None, approved, PolicyContext::Cli);
    if let Err(e) = pf.check(&fw.executor.policy(PolicyContext::Cli)) {
        println!("BLOCKED: {e}");
        if pf.risk >= RiskLevel::High { println!("try: run --approve {target}"); }
        return;
    }
    ...
    match fw.executor.execute(l, &target, None, approved, PolicyContext::Cli, Some(jid.as_u64())).await {
        Ok(r) => {
            fw.jobs.complete(jid, r.clone());
            println!("job {jid} completed");
            if let Some(ref sid) = r.session_id {
                let kind = if sid.starts_with("session:") { SessionKind::Shell } else { SessionKind::Unknown };
                let sid2 = fw.sessions.register(Session::new(kind, &target, &l.info.name));
                println!("session {sid2} opened");
            }
            println!("{}", serde_json::to_string_pretty(&r).unwrap());
        }
        Err(e) => { fw.jobs.cancel(jid); println!("error: {e}"); }
    }
}
...
fn cmd_policy(a: &[&str], s: &CliState, fw: &mut Framework) {
    ...
    let target = a
        ...
        .unwrap_or_default();
    let Some(loaded) = icebox_modules::load(&module) else {
    ...
    let intents: Vec<&str> = loaded.info.effective_intents().iter().map(|i| i.as_str()).collect();
    println!(
        "intent: {}  (declared: {})",
        intents.join(", "),
...
fn cmd_policy_add(rest: &[&str], fw: &mut Framework) {
    ...
    let Some(kind) = rest.first().copied() else {
        println!("usage: policy rule add <deny|allow|maxrisk|deny-cvss|require-approval-if> <args>");
        return;
    };
        ...
        "require-approval-if" => {
            ...
            while i < rest.len() {
                match rest[i] {
                    ...
                    "--kev" => { kev = true; i += 1; continue; }
                    _ => {}
                }
                i += 1;
            }
            PolicyRule::RequireApprovalIf { cvss_above, epss_above, kev }
        },
        other => {
            println!("unknown rule kind: {other} (use deny|allow|maxrisk|deny-cvss|require-approval-if)");
            return;
        }
    };
    fw.executor.policy_set.add_rule(rule.clone());
    println!("added rule: {} (policy version now {})", policy_rule_str(&rule), fw.executor.policy_set.version);
}
...
fn cmd_policy_remove(rest: &[&str], fw: &mut Framework) {
    ...
    match fw.executor.policy_set.remove_rule(idx) {
        Some(r) => println!("removed [{}]: {} (policy version now {})", idx, policy_rule_str(&r), fw.executor.policy_set.version),
...
fn policy_rule_str(r: &PolicyRule) -> String {
    match r {
        ...
        PolicyRule::RequireApproval { capability, target_pattern } => {
            format!("require approval for capability {} on targets matching {}", capability.as_str(), target_pattern)
        }
        PolicyRule::DenyIfCvssAbove(t) => format!("deny if CVSS above {t}"),
        PolicyRule::RequireApprovalIf { cvss_above, epss_above, kev } => {
            let mut parts = Vec::new();
            if let Some(c) = cvss_above { parts.push(format!("cvss>{c}")); }
            if let Some(e) = epss_above { parts.push(format!("epss>{e}")); }
            if *kev { parts.push("kev=true".into()); }
...
async fn cmd_pack(a: &[&str], fw: SharedFramework) {
    match a.first().copied() {
        ...
        Some("apply") => {
            ...
            println!("pack applied: {name} (policy now v{})", g.executor.policy_set.version);
        }
...
async fn cmd_approve(a: &[&str], fw: SharedFramework) {
    match a.first().copied() {
        ...
        Some("approve") => {
            ...
            let mut loaded = match icebox_modules::load(&req.module) {
                Some(l) => l,
                None => {
